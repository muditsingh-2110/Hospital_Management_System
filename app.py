...
# We define the credentials once here to avoid repeating them.
# This does NOT create a connection.
DB_CONFIG = {
...
def reception_dashboard():
    db = mysql.connector.connect(**DB_CONFIG)
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT ep.uhid, ep.emp_id, e.name as emp_name, ep.symptoms, ep.queue_no, ep.doctor_assigned FROM employee_patient ep JOIN employee e ON ep.emp_id = e.emp_id")
...
def doctor_dashboard():
    ...
    cursor.execute("""
        ...
        FROM employee_patient ep JOIN employee e ON ep.emp_id = e.emp_id LEFT JOIN doctor d ON ep.doctor_assigned = d.name WHERE ep.status IS NULL
        UNION
        SELECT nep.uhid, nep.name, nep.symptoms, nep.queue_no, d.department
        FROM nonemployee_patient nep LEFT JOIN doctor d ON nep.doctor_assigned = d.name WHERE nep.status IS NULL
        ORDER BY queue_no
    """)
...
def pharmacy_dashboard():
    ...
    cursor = db.cursor(dictionary=True)
    cursor.execute("""
        SELECT uhid, prescription FROM employee_patient WHERE prescription IS NOT NULL AND status = 'examined'
        UNION
        SELECT uhid, prescription FROM nonemployee_patient WHERE prescription IS NOT NULL AND status = 'examined'
    """)
